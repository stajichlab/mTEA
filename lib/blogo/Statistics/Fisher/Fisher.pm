...
package Statistics::Fisher::Fisher;


use Statistics::Fisher::2D;
use strict;
use Carp;
use warnings;
...
sub getValues
{
  ...
  if(!(Statistics::Fisher::2D::computeMarginalTotals($values)) ){
    return;
  }
  ...
  if( !(Statistics::Fisher::2D::computeObservedValues($values)) ) {
      return;
  }
